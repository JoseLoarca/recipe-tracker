...
**Implemented (Milestone 6):** the full task orchestration (download → transcribe → extract → macro lookup → persist), `app/worker/logging_context.py` (contextvar + logging filter), and `app/logging_config.py` writing structured JSON to both stdout and a rotated `backend/logs/pipeline.log`.

**Implemented (Milestone 7):** all four extraction stages now call real services — yt-dlp (`download`), faster-whisper (`transcribe`, model loaded once per worker process and reused), the official `ollama` client via `app/services/ollama_client.py` validated against `app/core/extraction_schema.py` (`extract`), and USDA FoodData Central via `app/services/usda_client.py` with an LLM-estimate fallback (`macros`). `persist` was already real, writing through the same models and macro-reconciliation logic as the REST API. Verified end-to-end against the real host-native Ollama instance and the real USDA API (see PLAN.md §12 for the real bugs this surfaced), not just mocks.
...
Retry policy: max 2 retries with exponential backoff (`app/core/retry.py`) only on transient stages (download, USDA lookup) — see §12 for why this is a plain in-process helper rather than Celery's `autoretry_for`. Transcription/extraction failures are NOT auto-retried (likely deterministic) — user must resend. Logs are structured JSON, written to a volume-mounted, rotated file (`backend/logs/pipeline.log`) plus stdout.
...
- **Submission (implemented, Milestone 8):** any non-command text message that looks like a supported YouTube link (`app/core/video_url.py`) → bot creates the `recipes` row + enqueues `process_recipe_submission` with a fresh `correlation_id` → immediate ack ("Queued! Ref: ..."). Unrecognized text is silently ignored (a private, single-purpose bot gets stray messages; only real submissions are worth acknowledging). On success, the worker calls `bot/notifier.py` to send "Done! `<name>` is ready: `<link>`" (linking into the web UI via `FRONTEND_BASE_URL`). On failure, it sends the specific human-readable reason. Either way the notification is best-effort — a send failure is logged, never raised, since the recipe's own `status` is the source of truth.
...
**Milestone 8 note:** the previously planned internal-only endpoint for pipeline-created pending recipes was not built — see §12 for why. The bot creates pending recipes directly via `app.services.recipe_service.create_pending_recipe`, exactly like every other bot handler (registration, household, login) already reads/writes the database in-process rather than through the REST API.
...
**Implemented (Milestone 9):** a global coverage gate (`--cov=app --cov=bot --cov-fail-under=90`, also set in `pyproject.toml`'s `[tool.coverage.report]` for local runs) rather than the originally planned per-package threshold — simpler to reason about, and the suite already sits at ~96% including `bot/`, once the previously-untested command handlers (`start`, `household`, `login`, `main.build_application`) got the same MagicMock/AsyncMock-based unit tests as `submit`'s. Frontend coverage tooling/thresholds were **not** added this pass — the frontend has only 3 test files today (`App`, `MacroBadge`, `ShoppingList`) and adding a gate before growing that coverage would just fail CI; left for a future pass.
...
- **README.md:** overview/motivation → architecture diagram → real screenshots of the list/detail pages (desktop + mobile, see §12) → tech stack table → prerequisites (Docker, host Ollama + pulled model, own Telegram bot token via BotFather, own USDA API key, Tailscale free account) → quickstart (`.env` setup → `docker compose up -d` → message your bot to register → create/join a household → log into the web UI) → usage walkthrough → testing → project structure → ADR index → roadmap/non-goals → license. Explicitly documents that this repo is meant to be **self-hosted per household** — no shared/public instance exists or is intended.
...
7. ✅ Real pipeline stages: download (yt-dlp) → transcribe (faster-whisper) → extract (ollama-python client + schema validation) → USDA macros (+ LLM-estimate fallback).
...
- **Stage-level retry instead of Celery `autoretry_for` →** the originally planned retry mechanism (Milestone 6's §12 entry) assumed `autoretry_for` on a `@task`, but `download`/`transcribe`/`extract`/`macros` are plain functions called in sequence inside one task, not separate tasks — `autoretry_for` has nothing to attach to at that granularity. Built `app/core/retry.py::retry_with_backoff` instead: a small manual retry loop (max 2 retries, exponential backoff) called from within `download_audio` and `usda_client.lookup_food_macros`, applied only to genuinely transient exception types (`yt_dlp.utils.DownloadError` for unclassified failures, `httpx.TransportError`) so permanent failures (private/unavailable video, no USDA match) still fail fast without wasting retries.
- **Ollama sometimes omits `portion_count` →** a real bug caught testing `extract_recipe` against the actual local model (not the unit tests, which mock the Ollama response): when a transcript never states a serving size, the model returns `"portion_count": null`, and the original schema (`Field(gt=0)`, no default) rejected the entire otherwise-valid extraction as malformed, wasting the one stricter-prompt retry and then failing the whole submission. Fixed with a `field_validator(mode="before")` on `ExtractionSchema.portion_count` that defaults `None` to `1`.
- **USDA search's top hit can lack basic macros →** a real bug caught testing `usda_client.lookup_food_macros` against the real API (e.g. querying "chicken breast" against USDA's `DEMO_KEY`): the single top search result is sometimes a `Foundation`-dataset entry whose `foodNutrients` array doesn't include calories/protein/carbs/fat at all (they appear to be reported under a different structure for that dataset), even though later results for the same query have them. The original code only ever looked at `foods[0]` and returned `None` (falling straight to an LLM estimate) even when a good verified match existed two results down. Fixed by fetching the top 5 candidates and returning the first one with all four tracked nutrients present, rather than trusting the top search hit alone.
- **No internal endpoint for pipeline-created recipes →** the Milestone 6/7 note planning a separate `X-Internal-Key`-protected REST endpoint for the bot/worker to create pending recipes turned out unnecessary: every other bot handler (registration, household, login) already reads/writes the database directly via `SessionLocal()` + a service function, in-process, since the bot and backend are the same codebase/deployment — there was never a network boundary between them to protect with a shared secret. The submission handler (`bot/handlers/submit.py`) follows the same pattern, calling `recipe_service.create_pending_recipe` directly. `INTERNAL_API_KEY` was removed from config/`.env.example` since nothing used it.
- **Worker container missing the `bot` package →** a real bug caught only by a full Docker Compose smoke test (not by any test suite, which imports both `app` and `bot` from the same host checkout): `app/worker/tasks.py` now imports `bot.notifier` to send completion/failure notifications, but `Dockerfile.worker` only ever `COPY`'d `app/` into the worker image, not `bot/` — the worker crashed on startup with `ModuleNotFoundError: No module named 'bot'` the moment a real `celery worker` process tried to import the task module. Invisible to `pytest` because it runs against the full repo checkout on the host, never the trimmed-down image `Dockerfile.worker` actually builds. Fixed by adding `COPY bot ./bot` to `Dockerfile.worker`, alongside the existing `app` copy.
- **Coverage gate scoped to backend only →** Milestone 9 added a real `--cov-fail-under=90` CI gate, but only for `backend/` (`app` + `bot`). The frontend has just 3 test files today and no coverage tooling installed at all — bolting a threshold on now would only fail CI on pre-existing gaps, not catch regressions. Revisit once the frontend's page-level components (`RecipeListPage`, `RecipeDetailPage`, `RecipeEditPage`, `LoginPage`) have real test coverage.
- **README screenshots are real renders, not mockups →** taken by seeding real DB rows through the actual service layer (`create_recipe`, real macro reconciliation), logging into the actual running frontend via the real auth-code exchange, and driving headless Chrome over the DevTools Protocol to capture the actual rendered pages — not hand-drawn or AI-generated mockups. The demo data was deleted from the dev database afterward; nothing in the screenshots reflects real user data.
